...
class SaddleClimb:
    ...
    def get_hessian_climb_vec(self: None,
                        B: np.ndarray,
                        g: np.ndarray,
                        avg_path: np.ndarray,
                        ) -> np.ndarray:

        norm_avg_path = avg_path / LA.norm(avg_path)
        norm_g = g / LA.norm(g)
        norm_avg_path *= np.sign(np.dot(norm_avg_path, norm_g))
        eig, vec = LA.eigh(B)
        print('eig0: ', eig[0])
        uphill_vec = np.zeros(np.shape(vec))
        for i in range(len(eig)):
            sign = np.sign(np.dot(vec[:, i], norm_g))
            uphill_vec[:, i] = sign * vec[:, i]
        uphill_vec_path_dot = mult(uphill_vec.T, norm_avg_path)
        for i in range(len(eig)):
            if uphill_vec_path_dot[i] > 0:
                climb_vec = uphill_vec[:,i]
                break
        return climb_vec


    def mix_climb_directions(self, avg_path, min_mode, g):
        norm_avg_path = avg_path / LA.norm(avg_path)
        norm_min_mode = min_mode / LA.norm(min_mode)
        norm_g = g / LA.norm(g)  
        norm_avg_path *= np.sign(np.dot(norm_avg_path, norm_g))
        #norm_min_mode *= np.sign(np.dot(norm_avg_path, norm_min_mode))
       
        angle_avg = np.arccos(np.dot(norm_avg_path, norm_g))
        angle_min = np.arccos(np.dot(norm_min_mode, norm_g))

        angle_min_avg = np.arccos(np.dot(norm_min_mode, norm_avg_path))
        
        invsum = (1 / angle_avg) + (1 / angle_min)
        alpha = 1 / angle_avg / invsum
        beta = 1 / angle_min / invsum
        print('angle_avg: ', angle_avg * 57.3,
                ', angle_min: ', angle_min * 57.3,
                ', angle_min_avg: ', angle_min_avg * 57.3)
        print('alpha: ', alpha, ', beta: ', beta)
        mixed_path = alpha * norm_avg_path + beta * norm_min_mode
        return mixed_path
    ...
    def get_step(self, B, g, pos_1D):
       ...
       elif eigs_B[0] < 0: # and np.dot(avg_path_uphill, mode0_uphill) > 0:
           eigs_tmp, vecs_tmp = eigs_B.copy(), vecs_B.copy()
           print('climbing minimum eigenvector')
       else:
